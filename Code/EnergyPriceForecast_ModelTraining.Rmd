...
```{r}
ARIMA_model1 <- auto.arima(time_series_data, stepwise = FALSE, approximation = TRUE, d = 1, max.p = 3, max.q = 3, max.P = 2, max.Q = 2, seasonal = FALSE)
```
...
```{r}
SARIMA_model1 <- auto.arima(time_series_data, stepwise = FALSE, approximation = TRUE, d = 1, max.p = 3, max.q = 3, max.P = 2, max.Q = 2, seasonal = TRUE)
```
...
Generation_Overall_Output <- Generation[, -c(56, 57)] %>%
  ...
  # calculates the total energy output for (that trading date and period) each group by summing the energy output values across all sites
    .groups = "drop"
  )
...
# Season:
# I'm using meteorological season start dates (which are the same every year) for simplicity and consistency
...
Fitting model:
```{r}
ARIMAX_model1 <- auto.arima(time_series_data, xreg = xreg, stepwise = TRUE, approximation = TRUE, d = 1, max.p = 2, max.q = 2, max.P = 1, max.Q = 1, allowdrift = TRUE)
```
...
curve(dsnorm(x, mean = 0, sd = sd(residuals(fit_snorm)), xi = coef(fit_snorm)["skew"]), add = TRUE, col = "red")
...
curve(dged(x, mean = 0, sd = sd(residuals(fit_ged)), nu = coef(fit_ged)["shape"]), add = TRUE, col = "red")

x <- residuals(fit_std)
plot(density(residuals(fit_std)), main = "Residuals vs STD")
curve(dstd(x, mean = 0, sd = sd(residuals(fit_std)), nu = coef(fit_std)["shape"]), add = TRUE, col = "red")
...
curve(dsstd(x, mean = 0, sd = sd(residuals(fit_sstd)), nu = coef(fit_sstd)["shape"]), add = TRUE, col = "red")
...
curve(dged_log(x, mean = 0, sd = sd(residuals(fit_ged)), nu = coef(fit_ged)["shape"]), add = TRUE, col = "red")
```
...
# Note that the final parameter set (g1 = 2, g2 = 2, p = 2, q = 2) was excluded from the grid search due to the model's excessive complexity, which prevented the optimizer from inverting its Hessian matrix
...
# Labeling each model tested and printing the label to keep track
  model_name <- paste0("ARMA(", p, ",", q, ") GARCH(", g1, ",", g2, ")")
...
# Returns NULL if fitting the model fails
  fit <- tryCatch(
    fit_ARIMA_GARCH("ged", g1, g2, p, q),
    error = function(e) {
      cat("Error:", e$message, "\n")
      return(paste("Error: ", model_name))
    },
    warning = function(w) {
      cat("Warning:", w$message, "\n")
      return(paste("Warning: ", model_name))
    }
  )
...
```{r}
tbl <- imap_dfr(results, function(ic, model_name) {
  tibble(
    Model = model_name,
    Akaike = ic["Akaike", ],
    Bayes = ic["Bayes", ],
    Shibata = ic["Shibata", ],
    Hannan_Quinn = ic["Hannan-Quinn", ]
  )
})
sorted_tbl <- tbl %>% arrange(Akaike)
print(sorted_tbl)
```

The best ARIMA-GARCH model is ARMA(1,0) GARCH(1,2). It has the lowest AIC and the lowest Shibata. It has the second lowest Hannan-Quinn. It also has the fourth lowest BIC.

I will be moving forward with the ARMA(1,0) GARCH(1,2) model for forecasting.
...
Model for forecasting Energy_generation values:
```{r}
...
ARIMA_model_generation <- auto.arima(ts_Energy, stepwise = FALSE, approximation = TRUE, d = 1, max.p = 3, max.q = 3, max.P = 2, max.Q = 2, seasonal = TRUE)
```

```{r}
summary(ARIMA_model_generation)
```
Model for forecasting energy generation is ARIMA(2,1,1)(2,1,0)[48].

```{r}
# Energy generation output
Energy_generation <- forecast(ARIMA_model_generation, h = 17520)$mean

typeof(Energy_generation)
```
...
# Correcting issue with colnames:
...
acc_list <- list(
  ...
  ARIMAX    = ARIMAX_acc,
  ARIMAX2   = ARIMAX2_acc,
  ARIMA_GARCH = GARCH_acc)
```

Manually calculating MASE for ARIMA_GARCH:
```{r}
...
SARIMA and STL-ARIMA had similar performance with SARIMA having a slightly lower value for most metrics. STL-ARIMA had the lower value for ME and MPE.
