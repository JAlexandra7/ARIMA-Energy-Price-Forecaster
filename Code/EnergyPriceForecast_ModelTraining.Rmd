---
title: "Energy Price Forecasting: Model Training"
author: "J Alexandra"
date: "2025-07-11"
output: pdf_document
---
...
The ABY0111 point of connection refers to the location in the electricity network where power flows into or out of the Albury area. Albury is in the lower South Island.
...
```{r}
ndiffs(time_series_data)
```
The number of differences required to achieve stationarity is one.
...
## SARIMAX
I've chosen to fit an SARIMAX model to the energy price data because I want to see if incorporating external information will improve the models forecasting accuracy.
...
seasons_df <- sapply(Training_set$TradingDate, get_season)

Season <- data.frame(
  TradingPeriod = Training_set$TradingDate,
  Season = seasons_df
)
...
These engineered variables make up my xreg matrix for SARIMAX. I'm going to see if including these eXogenous variables in the model results in better forecasting accuracy.
...
Fitting model:
```{r}
SARIMAX_model1 <- auto.arima(time_series_data, xreg = xreg, stepwise = TRUE, approximation = TRUE, 
...
```
My SARIMAX model is: ARIMA(2,1,2)(0,0,1)[48] errors
...
## SARIMA-GARCH models
I've decided to fit an SARIMA-GARCH model because it can handle seasonality, volatility clustering, conditional heteroskedasticity and capture tail behavior. This combination could make it better at capturing long term patterns and dealing with sudden market shocks than an ARIMA model.

Fitting an SARIMA-GARCH model: 
...
```{r testing SARIMA-fiGARCH}
sarima_residuals <- residuals(SARIMA_model1)

fit_SARIMA_GARCH <- function(distribution, garchorder_1, garchorder_2, armaOrder1, armaOrder2){
spec <- ugarchspec(
...
ugarchfit(spec = spec, data = sarima_residuals, solver = "hybrid")
}

```

```{r}
fit_norm <- fit_SARIMA_GARCH("norm", 1, 1, 1, 1)
fit_std  <- fit_SARIMA_GARCH("std", 1, 1, 1, 1)
fit_ged <- fit_SARIMA_GARCH("ged", 1, 1, 1, 1)
fit_snorm <- fit_SARIMA_GARCH("snorm", 1, 1, 1, 1)
fit_sstd  <- fit_SARIMA_GARCH("sstd", 1, 1, 1, 1)
```
...
The Skewed Student's t-distribution (SSTD) has the best model fit according to AIC, BIC, Shibata, and Hannan-Quinn. I'm going to double check that its the best distribution for the data by looking at the residuals.
...
This shows that the theoretical distribution has much more variability (spread) than the actual distribution of the residuals. The gap between the curves suggests that the model with the ged distribution does not accurately capture the error structure.
...
For both plots, the peak of the actual residuals (shown by black line) is much higher than the theoretical distribution's peak (shown by red line). The SSTD theoretical distribution appears to more accurately capture the structure of the errors than the std distribution.
...
The model with the SSTD distribution most accurately captures the structure of the errors, but it can't capture the peak (mode of the data) where majority of the data is concentrated.

```{r}
library(moments)
skewness(residuals(fit_sstd))
kurtosis(residuals(fit_sstd))
```
The skewness is 1.8, meaning the residuals are strongly right skewed.

The kurtosis is much larger than 10, which mean that the residuals are extremely leptokurtic.

```{r}
# log-transformed SSTD density
...
plot(density(log(abs(residuals(fit_sstd)))), main = "Log Density of Residuals vs Theoretical SSTD")
curve(dged_log(x, mean = 0, sd = sd(residuals(fit_sstd)), 
               nu = coef(fit_sstd)["shape"]), add = TRUE, col = "red")
```

This plot compares the log density of residuals (black curve) against the log of the theoretical SSTD distribution (red curve).

Although both curves peak around 2, the black curve has a sharper and higher peak than the red, this suggests that the observed residuals are more concentrated (have less spread) near their mode than the theoretical SSTD distribution predicts.

The black curve extends further into the left tail and with higher density than the theoretical SSTD would predict. This indicates tail risk, which is the increased likelihood of extreme negative errors that could lead to substantial model errors.
...
### SARIMA-Garch grid search
#### SARIMA-GARCH

I will be using the residuals from my SARIMA model
```{r}
sarima_residuals <- residuals(SARIMA_model1)
```

```{r}
fit_SARIMA_GARCH_model <- function(garchorder_1, garchorder_2, armaOrder1, armaOrder2, spec_model){
spec <- ugarchspec(
...
# Fits the model using custom function fit_SARIMA_GARCH
# Captures errors and warnings to avoid crashing the loop
# Returns NULL if fitting the model fails
  fit <- tryCatch(
    fit_SARIMA_GARCH_model(g1, g2, p, q, spec_model),
    error = function(e) {
      cat("Error:", e$message, "\n")
      return(NULL)
    },
...
Out of the 108 models considered the SARIMA(3,1,1)(0,0,1)[48] + FIGARCH(1,d,1) with ARMA(2,2) in variance model had the lowest AIC, BIC, Shibata and Hannan-Quinn.
...
I will be moving forward with theis model for forecasting due to it having the best performance in terms of AIC, BIC, Shibata and Hannan-Quinn.

The model is SARIMA(3,1,1)(0,0,1)[48] + FIGARCH(1,d,1) with ARMA(2,2) in variance.

The SARIMA(3,1,1)(0,0,1)[48] model captures the mean and seasonal structure of the time series. The residuals from that model have been used to fit a FIGARCH(1,d,1) volatility model, incorporating an ARMA(2,2) structure in the conditional variance equation.


#### Fitting SARIMA-fiGARCH

```{r SARIMA fiGARCH}
arima_residuals <- residuals(SARIMA_model1)
...
```{r}
GARCH_model <- ugarchfit(spec = spec, data = sarima_residuals, solver = "hybrid")
show(GARCH_model)
```
The long memory parameter for volatility, d (delta), is approximately 0.60582936, indicating persistent volatility.

The model is SARIMA(3,1,1)(0,0,1)[48] + FIGARCH(1, 0.61, 1), with AR(1) in the mean and a skewed Student t distribution.

## TBATS
I've decided to fit a TBATS model because it can handle multi-seasonality and doesn't require stationary data.
...
My current model is TBATS(0.373, {0,0}, 0.805, {<48,8>})

It applies a Box-Cox transformation with a lambda of 0.373 to the data, it has no autoregressive or moving average components, it has a trend of 0.805 and has seasonal periods 48 and 8.
...
## SARIMAX
Of my three SARIMAX variables, only two are known in advance every year (Is_Weekend and Season), and one isn't known in advance every year (Energy_generation). Because I want to simulate real world conditions during validation I am going to compare the results of two SARIMAX models, one with the actual Energy_generation values and one with forecasted Energy_generation values. This is to understand how sensitive the model is to uncertainty in external inputs.

### SARIMAX; actual Energy_generation
...
Forecasting:
```{r}
forecast_raw_SARIMAX_1 <- forecast(SARIMAX_model1, h = 17520, xreg = xreg_future)
```

```{r}
forecast_SARIMAX <- ts(forecast_raw_SARIMAX_1, start = c(2018, 1), frequency = 48)
```

### SARIMAX; forecasted Energy_generation
...
Forecasting:
```{r}
forecast_raw_SARIMAX_2 <- forecast(SARIMAX_model1, h = 17520, xreg = xreg_future2)
```

```{r}
forecast_SARIMAX2 <- ts(forecast_raw_SARIMAX_2, start = c(2018, 1), frequency = 48)
```

## SARIMA-fiGARCH
```{r}
forecast_raw_Garch <- ugarchforecast(GARCH_model, n.ahead = 17520)
forecast_GARCH <- ts(fitted(forecast_raw_Garch), start = c(2019, 1), frequency = 48)
```

## TBATS

```{r}
forecast_TBATS <- forecast(Tbats_model, h = 17520)
```
...
acc_list <- list(
  ...
  SARIMAX    = SARIMAX_acc,
  SARIMAX_model2 = SARIMAX2_acc,
  SARIMA_fiGARCH = GARCH_acc,
  TBATS = TBATS_acc)
```

Manually calculating MASE for SARIMA-fiGARCH:
```{r}
# Naive forecast errors from training set
naive_errors <- time_series_data  # Assuming naive one-step forecasts

# MAE from naive model
mae_naive <- mean(abs(naive_errors))

# MAE from the SARIMA-fiGARCH forecast
mae_garch <- mean(abs(Val_time_series - as.numeric(forecast_GARCH)))

# MASE for SARIMA-fiGARCH
...
The worst model was SARIMA-fiGARCH with the worst performance on every metric except for Theil's U. The SARIMA-fiGARCH had a Theil's U of 1 indicating that it performs equally to a naive forecast.
...
The SARIMAX model with the (ARIMA(3,0,1)(1,1,0)[48] with drift) forecasted energy generation variable in its xreg (labeled SARIMAX_model2), performed similarly to SARIMAX with the actual values for energy generation in its xreg. There is an average difference of 0.05 between the performances. This tells me that the use of an estimated Energy_generation variable does not harm model performance significantly.

TBATS performed worse than every other model except for SARIMA-fiGARCH on all metrics other than MPE. TBATS had the 4th best MPE out of the seven models.
...
The MASE for all models, except for SARIMA-fiGARCH, is less than 1 which means they beat a naive "yesterday's price" benchmark on average absolute error. On average, these models are producing more precise forecasts than simply carrying forward the last known value.
...
The ARIMA model did better than the STL-ARIMA, SARIMAX and SARIMA-fiGARCH models which so far shows that a more complex model does not equal a better performance, those other models were overfitting on the training data and not generalizing well enough to forecast the following year.
...
```{r}
saveRDS(ARIMA_model1, file = "Models/Initial models from training/arima_model.rds")
saveRDS(SARIMA_model1, file = "Models/Initial models from training/sarima_model.rds")
saveRDS(STL_ARIMA_model1, file = "Models/Initial models from training/stl_arima_model.rds")
saveRDS(SARIMAX_model1, file = "Models/Initial models from training/SARIMAX_model.rds")
saveRDS(GARCH_model, file = "Models/Initial models from training/garch_model.rds")
saveRDS(Tbats_model, file = "Models/Initial models from training/Tbats_model.rds")


# Saving SARIMAX with xreg (done because future xreg contains forecasted variable)
saveRDS(list(model = SARIMAX_model1, future_xreg = xreg_future2), "Models/Initial models from training/arima_with_xreg.rds")
saveRDS(fit_Energy, file = "Models/Initial models from training/Energy_generation_model.rds")
...
write.csv(xreg, "Saved_Datasets/xreg.csv", row.names = FALSE)
write.csv(xreg_future2, "Saved_Datasets/xreg_future.csv", row.names = FALSE)
...
saveRDS(forecast_ARIMA, file = "Saved_Forecasts/forecast_ARIMA.rds")
saveRDS(forecast_SARIMA, file = "Saved_Forecasts/forecast_SARIMA.rds")
saveRDS(forecast_STL, file = "Saved_Forecasts/forecast_STL.rds")
saveRDS(forecast_SARIMAX2, file = "Saved_Forecasts/forecast_SARIMAX.rds")
saveRDS(forecast_GARCH, file = "Saved_Forecasts/forecast_GARCH.rds")
saveRDS(forecast_TBATS, file = "Saved_Forecasts/forecast_TBATS.rds")
```
