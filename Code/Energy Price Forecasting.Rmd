...
There are 17472 observations (rows).
...
I am going to remove these observations from the dataset as I want to maintain a consistent daily structure (fixed periodicity) for ARIMA:
```{r}
...
```{r}
Daylight <- which(Training_set$TradingDate == "2017-09-24")
Training_set <- Training_set[-Daylight, ]
```
...
```
There are 34 observations where the DollarsPerMegawattHour value was larger than the 400, which in a data set of 17472 observations is very few.
...
```{r}
Daylight2 <- which(test_data$TradingDate == "2018-09-30")
test_data <- test_data[-Daylight2, ]
```
...
## ARIMA
```{r}
forecast_raw_ARIMA_1 <- forecast(ARIMA_model1, h = 17472)
```
...
```{r}
forecast12months <- forecast(STL_ARIMA_model1, h = 17472)
```
...
forecasting:
```{r}
forecast12months2 <- forecast(ARIMAX_model1, h = 17472, xreg = xreg_future)
```
...
Forecasting:
```{r}
forecast_tbats <- forecast(TBATS_model1, h = 17472)
...
There are a few spikes outside of the confidence bounds, particularly at lag 1 suggesting hort-term autocorrelation. This implies that the AR term (p) is slightly under-specified. I am going to try increasing the AR component to try and capture the remaining autocorrelation.
...
ARIMA 2nd model:
```{r}
forecast12months_A <- forecast(ARIMA_model2, h = 17472)
```
...
STL-ARIMA 2nd model:
```{r}
forecast12months_S <- forecast(STL_ARIMA_model2, h = 17472)
```
...
STL-ARIMA 3rd model:
```{r}
forecast12months_S3 <- forecast(STL_ARIMA_model3, h = 17472)
```
...
ARIMA 3rd model:
```{r}
forecast12months_A3 <- forecast(ARIMA_model3, h = 17472)
```
...
times_forecast <- seq(start_time, by = "30 min", length.out = 17472)
